...
export class UDP_Client_ID extends App_ID_Template{
    constructor(remote, local, app)
    {
        super(local, app, local.addr + ':' + local.port + '>' + remote.addr + ':' + remote.port);
    ...
    {
        return `${this._local.addr}:${this._local.port}`;
    }
    ...
    {
        return (this._local.addr === local.addr && this._local.port === local.port)
