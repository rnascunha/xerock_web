...
export class Byte_Array_Int64
{
    static to(data, big_endian = false, opt = {})
    {
        opt = {...{sep: ' ', aggregate: 0, pad_size: 0, pad_char: ' '}, ...opt};
        
        return  Byte_Array_Base._set_arr_option(Byte_Array_Int64.to_arr(data, big_endian, opt), opt);  
    }
    ...
    static to_arr(data, big_endian = false, opt = {})
    {
        ...
        for(let i = 0; i < len; i++)
            temp.push(Byte_Array_Base._set_data_opt(dv.getBigInt64(i * 8, big_endian), opt));
        ...
        if(remainder)
        {
            ...
            temp.push(Byte_Array_Base._set_data_opt(new DataView(arr.buffer).getBigInt64(0, big_endian), opt));
        }
    ...
    static is_int64_num(num)
    {
        return num >= -9223372036854775808 && num <= 9223372036854775807;
    }
    
    static is_int64_array(arr)
    {
        return arr.every(num => Byte_Array_Int64.is_int64_num(num));
    }
    
    static num_bytes_int64(){ return 8; }
}