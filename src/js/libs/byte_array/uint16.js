...
export class Byte_Array_UInt16
{
    ...
    static to_arr(data, big_endian = false, opt = {})
    {
        ...
        for(let i = 0; i < len; i++)
            temp.push(Byte_Array_Base._set_data_opt(dv.getUint16(i * 2, big_endian),opt));
    ...
    static from(string, big_endian = false, opts = {})
    {
        ...
        static is_uint16_num(num)
    {
        return num >= 0 && num <= 65535;
    }
    
    static is_uint16_array(arr)
    {
        return arr.every(num => Byte_Array_UInt16.is_uint16_num(num));
    }
        
    static num_bytes(){ return 2; }
}