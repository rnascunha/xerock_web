...
export class Byte_Array_Int8
{
    static to(data, opt = {})
    {
        opt = {...{sep: ' ', aggregate: 0, pad_size: 0, pad_char: ' '}, ...opt};
        return Byte_Array_Base._set_arr_option(Byte_Array_Int8.to_arr(data, opt), opt);
    }
    ...
    static from(string, opts = {})
    {
        ...
        if(!Byte_Array_Int8.is_int8_array(temp))
            throw new Byte_Array_Error(Error_Type.OUT_OF_BOUND, temp);
        
        let container = [];
        let arr = new Uint8Array(temp);
        arr.forEach(v => container.push(v));
        
        return container;
    }
    
    static is_int8_num(num)
    {
        return num >= -128 && num <= 127;
    }
    
    static is_int8_array(arr)
    {
        return arr.every(num => Byte_Array_Int8.is_int8_num(num));
    }
