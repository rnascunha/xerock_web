...
export class Byte_Array_Int32
{
    static to(data, big_endian = false, opt = {})
    {
        opt = {...{sep: ' ', aggregate: 0, pad_size: 0, pad_char: ' '}, ...opt};
        return  Byte_Array_Base._set_arr_option(Byte_Array_Int32.to_arr(data, big_endian, opt), opt);
    }
    ...
    static to_arr(data, big_endian = false, opt = {})
    {
        ...
        for(let i = 0; i < len; i++)
            temp.push(Byte_Array_Base._set_data_opt(dv.getInt32(i * 4, big_endian), opt));
        ...
        if(remainder)
        {
            ...
            temp.push(Byte_Array_Base._set_data_opt(new DataView(arr.buffer).getInt32(0, big_endian), opt));
        }
    ...
    static is_int32_num(num)
    {
        return num >= -2147483648 && num <= 2147483647;
    }
    
    static is_int32_array(arr)
    {
        return arr.every(num => Byte_Array_Int32.is_int32_num(num));
    }
    
    static num_bytes_int32(){ return 4; }
}